...
if (isset($_POST['payment_id'], $_POST['payment_date'], $_POST['bank'], $_POST['reason'], $_POST['price'], $_POST['sender'], $_POST['details'])) {
	...
	if ($stmt) {
		...
		$stmt->store_result();
		if ($stmt->num_rows < 1){
			header('Location: ./clientarea.php?action=edit_payment&status=invalid');
			$error_msg = "Database error.";
		}
	}
	else {
        header('Location: ./clientarea.php?action=edit_payment&status=error');
		$error_msg = "Database error.";
    }	
	
	$status = 'N';
	if (empty($error_msg)) {		    
  	  if ($update_stmt = $mysqli->prepare("UPDATE payments SET payment_date = ?, bank = ?, reason = ?, price = ?, sender = ?, details = ?, status = ? WHERE ((id = ?) and (member_id = ?))")) {
 	 		$update_stmt->bind_param('sssssssss', $payment_date, $bank, $reason, $price, $sender, $details, $status, $payment_id, $member_id );
		if (! $update_stmt->execute()) {
			header('Location: ./clientarea.php?action=edit_payment&status=error');
   	  	}
 	}
	header('Location: ./clientarea.php?action=payments&status=update_success');   
	}
}