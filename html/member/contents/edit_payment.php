...
if (!empty($id)) :	
	...
	$status = "'N'";	
	$stmt = $mysqli->prepare('SELECT id, payment_date, bank, reason, price, sender, details FROM payments where (id = '.$mysqli->real_escape_string($id).' and member_id = '.$mysqli->real_escape_string($member_id).' and status = '.$status.')'); 	
	if ($stmt) {
		$stmt->execute();
