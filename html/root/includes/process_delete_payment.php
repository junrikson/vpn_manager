...
if (isset($_POST['payment_id'])) {
	...
	if ($stmt) {
		...
		$stmt->store_result();
		if ($stmt->num_rows < 1){
			header('Location: ./index.php?action=delete_payment&status=invalid');
			$error_msg = "Database error.";
		}
	}
	else {
        header('Location: ./index.php?action=delete_payment&status=error');
		$error_msg = "Database error.";
    }	
	...
	header('Location: ./index.php?action=payments&status=delete_success');   
	}
}