...
if (isset($_POST['product_id'], $_POST['invoice_id'], $_POST['account_name'])) {
	...
	if ($stmt) {
		...
		$stmt->store_result();
		if ($stmt->num_rows < 1){
			header('Location: ./index.php?action=edit_invoice&status=invalid');
			$error_msg = "Database error.";
		}
	}
	else {
        header('Location: ./index.php?action=edit_invoice&status=error');
		$error_msg = "Database error.";
    }
	...
	header('Location: ./index.php?action=invoices&status=update_success');   
	}
}