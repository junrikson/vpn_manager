...
if (isset($_POST['username'], $_POST['password'], $_POST['details'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            header('Location: index.php?action=addaccount&status=duplicate');
			$error_msg = "A user with this email address already exists.";
        }
    } else {
        header('Location: index.php?action=addaccount&status=error');
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows > 4) {
            // A user with this email address already exists
            header('Location: index.php?action=addaccount&status=max');
			$error_msg = "A user with this email address already exists.";
        }
    } else {
        header('Location: index.php?action=addaccount&status=error');
    ...
    if (empty($error_msg)) {
       ...
       if ($insert_stmt = $mysqli->prepare("INSERT INTO accounts (username, password, member_id, expired, details, status) VALUES (?, ?, ?, ?, ?, ?)")) {
        ...
        header('Location: index.php?action=viewaccounts&status=success');
    }
}